...
/bin/bash ./scripts/githooks/git-pre-commit.sh
if [[ $? = 1 ]]; then
    all_pass=1
fi
...
/bin/bash ./scripts/githooks/git-pre-commit.sh  "My valid commit message"
if [[ $? = 0 ]]; then
    all_pass=1
fi
...
/bin/bash ./scripts/githooks/git-pre-commit.sh  "DR-11 My message"
if [[ $? = 0 ]]; then
    all_pass=1
fi
